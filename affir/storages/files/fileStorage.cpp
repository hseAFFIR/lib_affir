...
FileStorage::FileStorage(const std::string &filename, const size_t filesize) {
    ...
    LOG_DEBUG(Logger::logger, "Created file block with: filename = {}, startPos = {}, endPos = {}, id = {}",
...
void FileStorage::write(std::string_view data) {
    ...
    LOG_DEBUG(Logger::logger, "New pos (file {}) = {}", dataStruct.filename, (long)position);
}
...
size_t FileStorage::read(std::vector<char> &buffer, size_t bytesToRead, size_t startPos) {
    ...
    LOG_DEBUG(Logger::logger, "Read {} bytes from position {}", bytesRead, startPos);
    return bytesRead;
}
...
void FileStorage::loadStorageMeta() {
    ...
    if (!metaFileIn.is_open()) {
        LOG_WARNING(Logger::logger, "Metadata file storage hasn't been created yet: {}", std::string(strerror(errno)));
        return;
    }
